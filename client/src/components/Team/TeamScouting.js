...
export default function TeamScouting({ scouting }) {
  ...
  const bansAgainstTeamKeys = Object.keys(BannedAgainstTeam).sort((a, b) => BannedAgainstTeam[b] - BannedAgainstTeam[a]);
  const bansByTeamKeys = Object.keys(BannedByTeam).sort((a, b) => BannedByTeam[b] - BannedByTeam[a]);
  ...
  const sortedPlayerList = playerList.sort((a, b) => b.GamesPlayed - a.GamesPlayed);

...
seasonV1Routes.put('/roster/profile/add', authenticateJWT, (req, res) => {
  ...
  addProfilesToRoster(seasonId, teamPId, profileNameList).then((data) => {
...
/**
 * @route   PUT api/season/v1/roster/team/new
 * @desc    Creates a new Team(s), and adds them into the Season
 * @access  Private
 */
seasonV1Routes.put('/roster/team/new', authenticateJWT, (req, res) => {
...
/**
 * @route   PUT api/season/v1/roster/profile/new
 * @desc    Creates a new Player(s), and adds them into the Season's roster
 * @access  Private
 */
seasonV1Routes.put('/roster/profile/new', authenticateJWT, (req, res) => {
  const { opggUrlList, profileNameList, teamName, seasonShortName } = req.body;
  console.log(`PUT Request adding ${opggUrlList.length} new profile(s) to team '${teamName}' in season '${seasonShortName}'.`);

  addNewProfilesToRoster(opggUrlList, profileNameList, teamName, seasonShortName).then((data) => {
    if (data.errorMsg) { return res400sClientError(res, req, data.errorMsg, data.errorList); };
    return res200sOK(res, req, data);
